...
class TaskSyncUblv2(UserTrackingBase):

    def __init__(self, config):
        super(TaskSyncUblv2, self).__init__(config)
        ...
        self.blacklist = ['DOMContentLoaded', 'beforeunload', 'blur', 'change', 'focus', 'heartbeat',
                          'load', 'pagehide', 'pageshow', 'readystatechange', 'click', 'page_view']
    ...
    def extract(self):
        ...
        events = self.ublv2['events'].distinct(
            "event", {"_id": {"$gte": from_id, "$lt": to_id}}
        )
        for k in tqdm.tqdm(events):
            if k in self.blacklist:
                continue
            df_raw = pd.DataFrame(list(
                self.ublv2['events'].find({
    ...
    def load(self, df, k):
        from_date = self.from_date.replace(minute=0, second=0, microsecond=0)
        drop_cols = [k for k, v in df.isna().all().to_dict().items() if v]
        df = df.drop(columns=drop_cols)
        # step 3: upload gcs
        gcs_path = "user_tracking/ublv2/{}/{}".format(
            k, from_date.astimezone(sg_tz).strftime("%Y-%m-%d/%H%M.pq")
        )
    ...
    def backfill_execute(self, from_date, to_date, events, interval_hours=1):
        from_time = from_date
        seed_time = from_date + datetime.timedelta(hours=interval_hours)
        while from_time < to_date:
            to_time = seed_time
            # step 1: extract
            self.log.info("# step 1: extract")
            ...
            from_time = seed_time
            seed_time += datetime.timedelta(hours=interval_hours)
