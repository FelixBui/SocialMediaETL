...
class TaskCentralizeTopdevCompany(AppBase):

    company_cols = [
        'company_id', 'company_name', 'company_overview', 'company_url',  'address',
        'company_size', 'tech_stack', 'nationality', 'created_at', 'updated_at']
    ...
    def __init__(self, config):
        super(TaskCentralizeTopdevCompany, self).__init__(config)
    ...
    def transform(self, raw_df):
        ...
        company_df = raw_df[self.company_cols].rename(columns=self.mapping_company)

        if company_df.shape[0] > 0:
            company_df['address'] = company_df['address'].map(lambda x: [x] if isinstance(x, str) else x)

            # norm size
            company_df['size'] = company_df['size'].str.extract(r'(\d+ - \d+|\d+-\d+|\d+)')
            company_df['size'] = company_df['size'].map(
                lambda x: '1000 - 9999' if x == '1000' else x
            ...
            company_df['channel_code'] = 'TOPDEV'
    ...
    def load(self, company_df):
        insert_df_to_postgres(
            self.postgres_conf, tbl_name="topdev_company",
