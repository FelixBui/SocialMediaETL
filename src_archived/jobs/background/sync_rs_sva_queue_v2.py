...
class SyncRsSvaQueueV2(AppBase):
    ...
    def execute(self):
        ...
        while True:
            time_id = recsys_db['ubl_view_apply_raw_v2'].find_one({}, sort=[("_id", -1)])['_id'].generation_time
            checkpoint_id = ObjectId.from_datetime(time_id - datetime.timedelta(seconds=5))
            ...
            for e in it_raw:
                try:
                    ...
                    if recsys_db['ubl_view_apply_raw_v2'].find_one({"_id": x['_id']}) is None:
                        recsys_db['ubl_view_apply_raw_v2'].update_one(
                            {"_id": x['_id']},
                            {"$set": x}, upsert=True)
                    else:
                        continue
                ...
                if verb == 'view_job':
                    recsys_db['ubl_view_apply_v2'].update_one(
                        {
                            "channel_code": channel_code,
                            "exp_session_id": exp_session_id
                        },
