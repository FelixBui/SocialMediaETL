...
class TaskGa(AppBase):
    ...
    def get_metrics_by_channel(self,channel_name, start_date, end_date):
        # init report
        ...
        df_lst = list()
        # split to small group since limitation - ignore sessions metrics
        extract_metric = metrics[1:]
        ...
        for df in df_lst:
            df_base = pd.merge(df_base, df, how='left', on=['ga:campaign','ga:source','ga:medium'])
        df_full = df_base.copy()
        df_full['channel_name'] = channel_name
        return df_full

    def extract_by_channel_per_day(self, channel_name, date):
        df = self.get_metrics_by_channel(channel_name, date, date)
        df['start_date'] = date
    ...
    def transform_to_ga_metrics_schema(self, df):
        ...
        rs_df = rs_df[['source','medium','campaign','metric_name','metric_value','channel_name','date_start']]
        return rs_df
