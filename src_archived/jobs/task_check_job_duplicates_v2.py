...
from libs.db_utils import load_df_from_postgres, insert_df_to_postgres_fast, get_neo_client
from consolidated.normalize import build_network_duplicates_v2
...
class TaskCheckJobDuplicates(AppBase):
    ...
    def __init__(self, config):
        ...
        self.neo_driver = get_neo_client(self.get_param_config(['db', 'neo4j']))
    ...
    def transform(self, df_vnw_job, df_cb_job, df_topcv_job, df_raw_sv_job):
        ...
        df_job['global_id'] = df_job['id'] + "_" + df_job['channel_code']

    def build_graph(self, df_job):
        df_group = df_job.groupby(['global_company_id'])[
            ['global_id', 'token_name', 'areas', 'level', 'description']].agg(list).reset_index()
        block_process = 50
        with tqdm.tqdm(total=df_group.shape[0]) as pbar:
            for i in range(0, df_group.shape[0], block_process):
                ...
                tmp = _df.apply(
                    lambda x: build_network_duplicates_v2(
                        x['global_id'], x['token_name'], x['areas'], x['level'], x['description']
                    ),
                    axis=1
                )
                for e in tmp:
                    if len(e) > 0:
                        dct_rel.update(e)
                if len(dct_rel) > 0:
                    s1 = set(k[0] for k in dct_rel.keys())
                    s2 = set(k[1] for k in dct_rel.keys())
                    s = s1 | s2
                    df_job_ir = df_job.loc[df_job['global_id'].isin(s)]

                    with self.neo_driver.session() as session:
                        cql = """
                        UNWIND $dct AS dct
                        MERGE (j: `Job` {`id`: dct.global_id, `channel_code`: dct.channel_code})-[:created_by]-> 
                              (c: `Company` {`id`: dct.global_company_id})
                        SET j.name = dct.norm_name
                        """
                        dct = df_job_ir.to_dict(orient='records')
                        session.run(cql, dct=dct)
                        print("nodes already for create networking")

                        cql = """
                        UNWIND $dct AS dct
                        MATCH (s: `Job` {`id`: dct.s})
                        MATCH (t: `Job` {`id`: dct.t})
                        MERGE (s) -[:is_dupl {`score`: dct.score}]- (t)
                        """
                        dct = pd.DataFrame(
                            [(k[0], k[1], v) for k, v in dct_rel.items()],
                            columns=['s', 't', 'score']
                        ).to_dict(orient='records')
                        session.run(cql, dct=dct)
                        print("nodes already for build rel")
