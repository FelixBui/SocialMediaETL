...
class TaskSyncPtUblv2(UserTrackingBase):

    def __init__(self, config):
        super(TaskSyncPtUblv2, self).__init__(config)
    ...
    def load(self, df, k):
        ...
        gcs_path = "user_tracking/ublv2/{}/{}".format(
            k, self.from_date.astimezone(sg_tz).strftime("%Y-%m-%d/%H%M.pq")
        )
