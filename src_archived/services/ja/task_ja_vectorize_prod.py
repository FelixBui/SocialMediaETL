...
from libs.storage_utils import load_df_from_bq, load_sa_creds, insert_df_to_postgres, split_dataframe

local_tz = tz.tzlocal()
utc_tz = tz.tzutc()


class TaskJaVectorizeProd(AppBase):

    def __init__(self, config):
        super(TaskJaVectorizeProd, self).__init__(config)
    ...
    def extract_res(self, from_date, to_date, query_col='update_ts'):
        ...
        select
          ...
          resume.update_ts
        from
          `dwh_prod.src_adv2_seeker_resume` resume
        left join `dwh_prod.src_adv2_seeker_seeker` seeker on resume.seeker_id = seeker.id
        ...
        resume_df = load_df_from_bq(self.bq_creds, res_sql)
        return resume_df
    ...
    def extract_res_exp(self, from_date, to_date, query_col='update_ts'):
        ...
        select
          ...
          case
           when (experience.start_date is null
                 or experience.start_date < '1900-01-01') then timestamp '1970-01-01'
           when (experience.start_date > '2022-01-01') then current_timestamp
           else cast(experience.start_date as timestamp)
          end start_date,
          experience.position,
        ...
        from
          dwh_prod.src_adv2_seeker_resume resume
          join src_raw.adv2_seeker_resume_experience as experience 
        ...
        df_raw = load_df_from_bq(self.bq_creds, res_exp_sql)
        return df_raw
    ...
    def extract_job(self, from_date, to_date, query_col='update_ts'):
        ...
        select
          ...
          updated_at,
          update_ts
        from dwh_prod.src_adv2_emp_job job
        where channel_code='tvn'
          and job.{0} >= '{1}' and job.{0} < '{2}' 
          and job.status=1
        """.format(query_col, from_date, to_date)
        self.log.info(job_sql)
        df_raw = load_df_from_bq(self.bq_creds, job_sql)
        return df_raw
