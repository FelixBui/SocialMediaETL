...
from libs.storage_utils import load_df_from_postgres, upload_df_to_gcs, get_gcs_cli, \
    load_df_from_mysql, get_rmq_channel, get_rmq_cli, push_rmq_channel
...
class TaskJaPredictQc(AppBase):

    def __init__(self, config):
        super(TaskJaPredictQc, self).__init__(config)
    ...
    def extract(self):
        ...
        JOIN erp_sales_order.running_job_box run on job.channel_code=run.channel_code and job.id = run.job_id
        ...
        FROM erp_seeker.resume resume
        WHERE resume.id in {}
        """

        filtered_resume_sql = """
        SELECT DISTINCT resume_id resume_id
        FROM ja_roll_out
        WHERE status=1
        """
        ...
        df_job_active = load_df_from_mysql(self.mysql_conf, job_sql)
        filtered_resume_df = load_df_from_postgres(self.postgres_conf, filtered_resume_sql)
        q = resume_sql.format('({})'.format(', '.join(filtered_resume_df['resume_id'].map(str).tolist())))
        df_res_active = load_df_from_mysql(self.mysql_conf, q)
    ...
    def predict(self, df_app, df_job_active, df_res_active,
        ...
        with tqdm.tqdm(total=df_app.shape[0]) as pbar:
            for df_in_app in split_dataframe(df_app, self.chuck_size):
                ...
                df_selected = df_in[['seeker_id', 'job_id', 'resume_id']]
